...
int _nnls(double *a, int m, int n, double *b, double *x, double *rnorm, double *wp, double *zzp, int *indexp)
{
  ...
  while(iz1<=iz2 && nsetp<m) {
    ...
    for(int iz=iz1; iz<=iz2; iz++) {
      int ni=index[iz];
      double sm=0.;
      for(int mi=npp1; mi<m; mi++) sm+=a[ni*m+mi]*b[mi];
      w[ni]=sm;
    }
    ...
    while(1) {
      ...
      double asave=a[j*m+npp1];
      up=0.0;
      _lss_h12(1, npp1, npp1+1, m, &a[j*m+0], 1, &up, NULL, 1, 1, 0);
      double unorm=0.0;
      if(nsetp!=0) for(int mi=0; mi<nsetp; mi++) unorm+=a[j*m+mi]*a[j*m+mi];
      unorm=sqrt(unorm);
      double d=unorm+fabs(a[j*m+npp1])*0.01;
      if((d-unorm)>0.0) {
        /* Col j is sufficiently independent. Copy B into ZZ, update ZZ
           and solve for ztest ( = proposed new value for X[j] ) */
        for(int mi=0; mi<m; mi++) zz[mi]=b[mi];
        _lss_h12(2, npp1, npp1+1, m, &a[j*m+0], 1, &up, zz, 1, 1, 1);
        double ztest=zz[npp1]/a[j*m+npp1];
    ...
    if(iz1<=iz2)
      for(int jz=iz1; jz<=iz2; jz++) {
        jj=index[jz];
        _lss_h12(2, nsetp-1, npp1, m, &a[j*m+0], 1, &up, &a[jj*m+0], 1, m, 1);
      }
    if(nsetp!=m) for(int mi=npp1; mi<m; mi++) a[j*m+mi]=0.;
    w[j]=0.;
    
    /* Solve the triangular system; store the solution temporarily in Z[] */
    for(int mi=0; mi<nsetp; mi++) {
      int ip=nsetp-(mi+1);
      if(mi!=0) for(int ii=0; ii<=ip; ii++) zz[ii]-=a[jj*m+ii]*zz[ip+1];
      ...
      do {
        ...
        if(jj!=(nsetp-1)) {
          ...
          for(int ni=jj+1; ni<nsetp; ni++) {
            ...
            double ss, cc;
            _lss_g1(a[ii*m+(ni-1)], a[ii*m+ni], &cc, &ss, &a[ii*m+(ni-1)]);
            a[ii*m+ni]=0.0;
            for(int nj=0; nj<n; nj++) if(nj!=ii) {
              /* Apply procedure G2 (CC,SS,A(J-1,L),A(J,L)) */
              double temp=a[nj*m+(ni-1)];
